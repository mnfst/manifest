...
beforeAll(async () => {
  ...
  // Clear any pricing rows seeded by createTestApp() to avoid conflicts
...
describe('Routing E2E', () => {
  describe('Connect a provider → tiers are auto-assigned', () => {
    it('should connect OpenAI provider (no apiKey needed)', async () => {
  ...
  describe('Deactivate all providers', () => {
    it('should deactivate all providers and clear overrides', async () => {
      ...
      const res = await auth(
        api().post('/api/v1/routing/providers/deactivate-all'),
      ).expect(201);

      expect(res.body.ok).toBe(true);
    });

    it('should have all providers deactivated', async () => {
      const res = await auth(
        api().get('/api/v1/routing/providers'),
      ).expect(200);
    ...
    it('should have all overrides cleared', async () => {
      const res = await auth(
        api().get('/api/v1/routing/tiers'),
      ).expect(200);
    ...
    it('should reconnect providers for subsequent tests', async () => {
      await auth(api().post('/api/v1/routing/providers'))
        .send({ provider: 'openai' })
        .expect(201);
      await auth(api().post('/api/v1/routing/providers'))
  ...
  describe('Resolve endpoint', () => {
    ...
    it('should return tier and score for simple message', async () => {
    ...
    it('should return a model when providers are connected', async () => {
      const res = await bearer(
        api().post('/api/v1/routing/resolve'),
      )
        .send({ messages: [{ role: 'user', content: 'hi' }] })
      ...
      // The test user has openai+anthropic connected from earlier tests
      expect(res.body.model).not.toBeNull();
      expect(res.body.provider).not.toBeNull();
    });

    it('should accept tools in the request', async () => {
      const res = await bearer(
        api().post('/api/v1/routing/resolve'),
      )
        .send({
      ...
      // Tools should force at least standard tier
      expect(res.body.tier).not.toBe('simple');
    });

    it('should accept recentTiers for momentum', async () => {
      const res = await bearer(
        api().post('/api/v1/routing/resolve'),
      )
        .send({
