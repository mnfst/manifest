...
describe('LimitCheckService', () => {
  ...
  describe('email notification on block', () => {
    ...
    it('resolves notification email from local config in local mode', async () => {
      const originalMode = process.env['MANIFEST_MODE'];
      process.env['MANIFEST_MODE'] = 'local';
      (readLocalNotificationEmail as jest.Mock).mockReturnValue('local-user@test.com');
      mockGetFullConfig.mockResolvedValue(null);
      mockQuery
        .mockResolvedValueOnce([]) // notification_logs check
        .mockResolvedValueOnce(undefined); // INSERT notification_logs
  ...
  describe('cache eviction', () => {
    it('evicts expired consumption cache entries on next fetch', async () => {
      mockGetActiveBlockRules.mockResolvedValue([
        {
          id: 'r1',
          tenant_id: 'tenant-1',
      ...
      // Manually expire the cache entry by manipulating internal state
      const consumptionCache = (service as any).consumptionCache as Map<
        string,
        { data: number; expiresAt: number }
      >;
      for (const [, entry] of consumptionCache) {
        entry.expiresAt = 0; // expire immediately
      }

      await service.checkLimits('tenant-1', 'my-agent');
      // Should refetch since cache was expired and evicted
      expect(mockGetConsumption).toHaveBeenCalledTimes(2);
    });
  });
});
