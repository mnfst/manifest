...
describe('scoreConditionalLogic', () => {
  ...
  it('returns positive for "if...then...otherwise"', () => {
    const text = 'if the user is admin then show dashboard, otherwise redirect';
  ...
  it('returns 0.6 for exactly two conditionals', () => {
    expect(scoreConditionalLogic('unless it fails, in case of error handle it')).toBe(0.6);
  });
...
describe('scoreConstraintDensity', () => {
  ...
  it('returns positive for constraint-rich text', () => {
    const text = 'at most 5 items, at least 2 required, exactly 3 columns, must not exceed limit';
    expect(scoreConstraintDensity(text)).toBeGreaterThan(0);
  });

  it('detects big-O notation', () => {
    const text = 'implement an algorithm with O(n log n) time complexity and O(1) space';
    expect(scoreConstraintDensity(text)).toBeGreaterThan(0);
  });
  ...
  it('returns 0 for text with no constraint patterns', () => {
    const text =
      'This is a long sentence with many words and lots of filler text to make it lengthy enough but nothing special going on here just a plain paragraph about everyday activities like walking running and swimming in the lake on a sunny afternoon during the summer months when the weather is nice.';
