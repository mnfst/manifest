...
describe('scoreRepetitionRequests', () => {
  ...
  it('returns positive for "give me 5 options"', () => {
    expect(scoreRepetitionRequests('give me 5 options')).toBeGreaterThan(0);
  });
  ...
  it('returns high score for 10+ variations', () => {
    expect(scoreRepetitionRequests('generate 15 examples')).toBeGreaterThanOrEqual(0.9);
  });
...
describe('scoreToolCount', () => {
  ...
  it('boosts score for specific tool_choice', () => {
    const tools = [{}];
    expect(scoreToolCount(tools, { type: 'tool', name: 'get_weather' })).toBeGreaterThan(
      scoreToolCount(tools),
    );
  });

  it('boosts for tool_choice "any"', () => {
    expect(scoreToolCount([{}], 'any')).toBeGreaterThan(scoreToolCount([{}]));
  });
  ...
  it('boosts for tool_choice "required"', () => {
    expect(scoreToolCount([{}], 'required')).toBeGreaterThan(scoreToolCount([{}]));
  });
