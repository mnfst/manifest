...
describe('scoreKeywordDimension', () => {
  ...
  it('scores formalLogic positively for "prove this theorem"', () => {
    ...
    const result = scoreKeywordDimension('formalLogic', matches, makeExtracted(text), 'up');
  ...
  it('scores analyticalReasoning for trade-offs', () => {
    ...
    const result = scoreKeywordDimension('analyticalReasoning', matches, makeExtracted(text), 'up');
    expect(result.rawScore).toBeGreaterThan(0);
  });

  it('scores codeGeneration for "write a function"', () => {
    const text = 'write a function to sort an array';
    const matches = scanWith('codeGeneration', ['write a function'], text);
    const result = scoreKeywordDimension('codeGeneration', matches, makeExtracted(text), 'up');
    expect(result.rawScore).toBeGreaterThan(0);
  });

  it('scores codeReview for "fix this bug"', () => {
    const text = 'fix this bug in my authentication code';
    const matches = scanWith('codeReview', ['fix this bug'], text);
    const result = scoreKeywordDimension('codeReview', matches, makeExtracted(text), 'up');
    expect(result.rawScore).toBeGreaterThan(0);
  });

  it('scores simpleIndicators NEGATIVELY (direction: down)', () => {
    const text = 'what is a dog';
    const matches = scanWith('simpleIndicators', ['what is'], text);
    const result = scoreKeywordDimension('simpleIndicators', matches, makeExtracted(text), 'down');
  ...
  it('scores relay NEGATIVELY (direction: down)', () => {
    ...
    const result = scoreKeywordDimension('relay', matches, makeExtracted(text), 'down');
    expect(result.rawScore).toBeLessThan(0);
  });
  ...
  it('position weighting: last message scores higher', () => {
    ...
    const lastMsg = scoreKeywordDimension('formalLogic', matches, makeExtracted(text, 1.0), 'up');
    const oldMsg = scoreKeywordDimension('formalLogic', matches, makeExtracted(text, 0.25), 'up');
  ...
  it('density bonus: 3+ keywords in 200-char window boosts score at lower position weight', () => {
    const denseText = 'prove theorem by induction and use deduction with axiom';
    const keywords = ['prove', 'theorem', 'induction', 'deduction', 'axiom'];
    ...
    const sparseText =
      'prove something. ' + 'x'.repeat(250) + ' theorem here. ' + 'x'.repeat(250) + ' induction.';
  ...
  it('scores across multiple extracted texts from different messages', () => {
    ...
    const result = scoreKeywordDimension('formalLogic', matches, extracted, 'up');
    expect(result.rawScore).toBeGreaterThan(0);
    expect(result.matchedKeywords).toContain('prove');
    expect(result.matchedKeywords).toContain('theorem');
  });
  ...
  it('clamps rawScore to [-1, 1] range', () => {
    ...
    const matches = scanWith('formalLogic', keywords, text);
    const result = scoreKeywordDimension('formalLogic', matches, makeExtracted(text, 1.0), 'up');
