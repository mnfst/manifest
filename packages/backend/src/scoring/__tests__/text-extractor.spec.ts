...
describe('extractUserTexts', () => {
  ...
  it('returns empty for system message only', () => {
    expect(extractUserTexts([{ role: 'system', content: 'You are a helper' }])).toEqual([]);
  });

  it('returns empty for developer message only', () => {
    expect(extractUserTexts([{ role: 'developer', content: 'instructions' }])).toEqual([]);
  });

  it('extracts single user message with string content', () => {
    const result = extractUserTexts([{ role: 'user', content: 'hello world' }]);
  ...
  it('gracefully handles content: null', () => {
    const result = extractUserTexts([{ role: 'user', content: null }]);
    expect(result).toHaveLength(0);
  });

  it('gracefully handles content: undefined', () => {
    const result = extractUserTexts([{ role: 'user', content: undefined }]);
    expect(result).toHaveLength(0);
  });
