    ...
    const result = scoreRequest(
      {
        messages: [
          {
            role: 'user',
            content: 'Write a comprehensive React application with multiple components',
          },
        ],
      },
      {
        ...
        messages: [
          {
            role: 'user',
            content:
              'Tell me about the general approach to this kind of thing and some related topics.',
          },
        ],
      },
      {
    ...
    const result = scoreRequest({
      messages: [
        { role: 'user', content: 99999 },
        {
          role: 'user',
          content:
            'Please explain this concept in detail with examples and use cases for production',
        },
      ],
    });
    expect(result).toHaveProperty('tier');
    ...
    const result = scoreRequest({
      messages: [
        { role: 'assistant', content: undefined },
        {
          role: 'user',
          content:
            'Write a detailed analysis of this codebase architecture and deployment strategy',
        },
      ],
    });
    expect(result).toHaveProperty('tier');
...
describe('scoreRequest — scoreStructuralDimension default branch', () => {
  it('returns 0 for unknown structural dimension names', () => {
    const result = scoreRequest(
      ...
      {
        dimensions: [{ name: 'unknownDimension', weight: 1.0, direction: 'up' as const }],
      },
    );
