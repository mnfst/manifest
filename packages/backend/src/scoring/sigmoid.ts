...
export function scoreToTier(score: number, boundaries: TierBoundaries): Tier {
...
export function computeConfidence(score: number, boundaries: TierBoundaries, k = 8): number {
  const boundaryValues = [boundaries.simpleMax, boundaries.standardMax, boundaries.complexMax];
