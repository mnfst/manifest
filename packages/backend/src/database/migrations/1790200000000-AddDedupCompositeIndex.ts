 ...
 * (tenant_id, agent_id, user_id, model, status='ok') and orders by timestamp
 * DESC, taking the 10 most recent rows. The leading columns match the equality
...
export class AddDedupCompositeIndex1790200000000 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE INDEX IF NOT EXISTS "IDX_agent_messages_tenant_agent_model_status_user_ts" ON "agent_messages" ("tenant_id", "agent_id", "model", "status", "user_id", "timestamp")`,
    );
  }
