...
export class TimeseriesQueriesService {
  ...
  async getPerAgentTimeseries(
    ...
    // Pivot: group by time bucket, one column per agent
    ...
    for (const r of rows) {
      ...
      byBucket.get(bucket)![String(r['agent_name'])] = Number(r['tokens'] ?? 0);
    }
  ...
  async getPerAgentCostTimeseries(
    ...
    for (const r of rows) {
      ...
      byBucket.get(bucket)![String(r['agent_name'])] = Number(r['cost'] ?? 0);
    }
  ...
  async getPerProviderTimeseries(
    ...
    for (const r of rows) providerSet.add(String(r['provider']));
    ...
    for (const r of rows) {
      ...
      byBucket.get(bucket)![String(r['provider'])] = Number(r['tokens'] ?? 0);
    }

    const timeseries = [...byBucket.entries()].map(([bucket, map]) => {
      const row: Record<string, number | string> = { [bucketAlias]: bucket };
      for (const p of providers) row[p] = map[p] ?? 0;
      return row;
    });
  ...
  async getPerProviderMessageTimeseries(
    ...
    for (const r of rows) {
      const bucket = String(r[bucketAlias]);
      if (!byBucket.has(bucket)) byBucket.set(bucket, {});
      byBucket.get(bucket)![String(r['provider'])] = Number(r['messages'] ?? 0);
    }
  ...
  async getPerProviderCostTimeseries(
    ...
    for (const r of rows) {
      const bucket = String(r[bucketAlias]);
      if (!byBucket.has(bucket)) byBucket.set(bucket, {});
      byBucket.get(bucket)![String(r['provider'])] = Number(r['cost'] ?? 0);
    }
    const timeseries = [...byBucket.entries()].map(([bucket, map]) => {
      const row: Record<string, number | string> = { [bucketAlias]: bucket };
      for (const p of providers) row[p] = map[p] ?? 0;
  ...
  async getPerModelCostTimeseries(
    ...
    for (const r of rows) {
      const bucket = String(r[bucketAlias]);
      if (!byBucket.has(bucket)) byBucket.set(bucket, {});
      byBucket.get(bucket)![String(r['model'])] = Number(r['cost'] ?? 0);
    }
    const timeseries = [...byBucket.entries()].map(([bucket, map]) => {
      const row: Record<string, number | string> = { [bucketAlias]: bucket };
      for (const m of models) row[m] = map[m] ?? 0;
