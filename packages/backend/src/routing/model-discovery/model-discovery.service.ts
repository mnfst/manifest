...
export class ModelDiscoveryService {
  ...
  async discoverModels(provider: UserProvider): Promise<DiscoveredModel[]> {
    ...
    if (provider.auth_type === 'subscription' && !apiKey) {
      ...
      // Keep confirmed-model filtering for API-key providers and curated
      // subscription catalogs for subscription-only providers.
      if (raw.length === 0) {
        raw =
          provider.auth_type === 'subscription'
            ...
            : buildFallbackModels(
                this.pricingSync,
                provider.provider,
                this.modelRegistry?.getConfirmedModels(provider.provider) ?? null,
              );
        if (raw.length > 0) {
          this.logger.log(
