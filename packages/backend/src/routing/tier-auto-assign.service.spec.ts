...
describe('TierAutoAssignService', () => {
  ...
  describe('pickBest', () => {
    ...
    it('defaults quality_score to 3 when null', () => {
      const nullQuality = makeModel({
        model_name: 'no-quality',
        quality_score: null as unknown as number,
      });
      const scored = service.pickBest([nullQuality], 'complex');
      // quality defaults to 3, so score output should be 3
      expect(scored!.score).toBe(3);
    });

    it('updates existing tier assignment on recalculate', async () => {
      const existing = {
        id: 'tier-1',
        user_id: 'user-1',
        tier: 'simple',
      ...
      // Return existing for all 4 tiers
      ...
      expect(existing.auto_assigned_model).toBe('gpt-4o');
    });
