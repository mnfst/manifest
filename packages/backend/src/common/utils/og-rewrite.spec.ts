...
<html>
  <head>
    <meta property="og:url" content="https://gateway.manifest.build" />
    <meta property="og:image" content="https://gateway.manifest.build/og-image.png" />
    <meta name="twitter:image" content="https://gateway.manifest.build/og-image.png" />
  </head>
</html>`;
...
describe('rewriteOgTags', () => {
  ...
  it('returns the input unchanged when baseUrl matches the default', () => {
    expect(rewriteOgTags(SAMPLE_HTML, 'https://gateway.manifest.build')).toBe(SAMPLE_HTML);
  });
  ...
  it('rewrites all occurrences of the default base to the custom base', () => {
    ...
    expect(result).not.toContain('https://gateway.manifest.build');
  });
