...
import { EMAIL_PROVIDER_OPTIONS, getEmailProviderOption } from '../services/email-providers.js';
...
const EmailProviderModal: Component<Props> = (props) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const name = getEmailProviderOption(provider())?.name ?? provider();
  ...
  const selectedProviderOption = () => getEmailProviderOption(provider());
  const keyDocsUrl = () => selectedProviderOption()?.apiKeyUrl ?? null;
        ...
        <div class="modal-overlay" onClick={() => props.onClose()}>
          ...
          >
            ...
            <div class="provider-modal-picker">
              <For each={EMAIL_PROVIDER_OPTIONS}>
                {(option) => (
                  <button
                    class="provider-modal-option"
                    classList={{ 'provider-modal-option--active': provider() === option.id }}
                    onClick={() => {
                    ...
                    type="button"
                  >
                    <img src={option.logoSrc} alt="" class="provider-modal-option__logo" />
            ...
            <Show when={keyDocsUrl()}>
              <p class="modal-card__field-hint">
                ...
                >
                  Get {selectedProviderOption()?.name ?? provider()} API key
                </a>
              </p>
            </Show>
