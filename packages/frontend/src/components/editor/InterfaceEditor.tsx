/**
 * InterfaceEditor - Full-screen code editor for Interface nodes.
 * Allows users to customize the TSX component code for their interface.
 */
...
/**
 * Full-screen editor for customizing Interface node component code.
 */
export function InterfaceEditor({
  ...
  // Track dirty state and trigger preview update
  const handleCodeChange = useCallback(
    (newCode: string) => {
      setCode(newCode);
      setIsDirty(newCode !== startingCode);
      // Increment preview key to trigger re-render after a debounce
    ...
    <div className="absolute inset-0 bg-background z-40 flex flex-col">
      ...
      <header className="flex items-center justify-between px-4 py-3 border-b bg-card">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-muted-foreground">
              Editing {layoutTemplate} component
        ...
        <div className="flex items-center gap-3">
          {/* Save button - styled consistently */}
          <button
            onClick={handleSave}
            disabled={!canSave}
            ...
            title={hasErrors ? 'Fix errors before saving' : 'Save (Cmd+S)'}
          >
