...
import { getCodexConfigSnippet } from '../services/framework-snippets.js';
...
const CodexSetup: Component<Props> = (props) => {
  ...
  const settingsCopy = () => getCodexConfigSnippet(props.baseUrl, copyKey());
  const settingsShown = () => getCodexConfigSnippet(props.baseUrl, visibleKey());

  return (
    <div class="setup-agents-card">
      <p class="setup-method__hint">
        Add this block to <code class="setup-model-hint__code">~/.codex/config.toml</code> and
        export <code class="setup-model-hint__code">OPENAI_API_KEY</code> — every{' '}
        <code class="setup-model-hint__code">codex</code> run will route through Manifest's{' '}
        <code class="setup-model-hint__code">/v1/responses</code> endpoint.
      </p>
      ...
      <div class="setup-cli-block">
        ...
        <CodeBlock code={settingsShown()} language="toml" />
      </div>
    </div>
  );
};
