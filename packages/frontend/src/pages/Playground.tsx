...
const Playground: Component = () => {
  ...
  const refreshHistory = async () => {
    ...
    try {
      setHistoryRuns(await listPlaygroundRuns(name));
    } catch (err) {
    ...
    <div class="playground" style={{ 'padding-bottom': `${promptHeight() + 48}px` }}>
      ...
      <Show when={!hasAgents() && agentList() !== undefined}>
        <div class="routing-no-providers">
          ...
          <span class="routing-no-providers__title">
            {(providers() ?? []).length === 0
              ? 'No harnesses or providers'
              : 'No harnesses created'}
          </span>
          <span class="routing-no-providers__desc">
            {(providers() ?? []).length === 0
              ? 'Create a harness and connect a provider to start using the Playground.'
              : 'Create a harness to start using the Playground.'}
          </span>
      ...
      <Show
        ...
        fallback={
          <Show when={hasAgents() && (available() !== undefined || providers() !== undefined)}>
