...
const Byok: Component = () => {
  ...
  return (
    <div class="container--lg">
      ...
      {(() => {
        const totalCost = () =>
          connectedProviders().reduce((s, p) => s + (getConnected(p.id)?.consumption_cost ?? 0), 0);
        return (
          <Show when={connectedRows().length > 0}>
            <div class="chart-card" style="margin-bottom: 24px; padding: 20px 24px;">
              <span class="chart-card__label" style="display: flex; align-items: center; gap: 0;">
                Total API cost (30d)
                <InfoTooltip text="Sum of all API key usage costs across your connected providers." />
              </span>
              <div class="chart-card__value-row" style="margin-top: 4px;">
                <span class="chart-card__value">{formatCost(totalCost()) ?? '$0.00'}</span>
              </div>
            </div>
          </Show>
        );
