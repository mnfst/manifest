...
const Byok: Component = () => {
  ...
  const [analytics] = createResource(
    () => chartRange(),
    (range) => getProviderAnalytics('api_key', range) as Promise<AnalyticsResponse>,
  );

  const [agentTimeseries] = createResource(
    () => chartRange(),
    (range) => getPerAgentTimeseries('api_key', '', range),
  );

  const [agentMessageTimeseries] = createResource(
    () => chartRange(),
    (range) => getPerAgentMessageTimeseries('api_key', '', range),
  );
