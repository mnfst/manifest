...
const RoutingSpecificitySection: Component<RoutingSpecificitySectionProps> = (props) => {
  ...
  const handleToggle = async (category: string, label: string, active: boolean) => {
    ...
    try {
      ...
      toast.error('Failed to update specificity tier');
    } finally {
      setToggling(null);
    }
  };
    ...
    <div class="routing-section">
      <div class="routing-section__header specificity-header">
        <div class="specificity-header__left">
          <span class="routing-section__title">Specificity tiers</span>
          <span class="routing-section__subtitle">
            Specificity tiers override generalist routing when a request matches a specific task
            type. Enable a tier and assign the best model for that job.
          </span>
        </div>
        <button class="btn btn--primary btn--sm" onClick={() => setShowModal(true)}>
          {hasAnyActive() ? 'Manage specificity tiers' : 'Enable specificity tiers'}
        </button>
      </div>
      ...
      <Show
        ...
        fallback={
          <div class="specificity-empty">
            <span class="specificity-empty__title">No specificity tier yet</span>
            <span class="specificity-empty__desc">
              Enable specificity tiers to route specialized tasks to dedicated models.
            </span>
            <button class="btn btn--primary btn--sm" onClick={() => setShowModal(true)}>
              Enable specificity tiers
            </button>
          </div>
        }
      >
