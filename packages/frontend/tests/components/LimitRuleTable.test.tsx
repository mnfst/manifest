...
describe('LimitRuleTable', () => {
  ...
  it('renders rules with aria-hidden on decorative SVGs', () => {
    ...
    const { container } = render(() => (
      <LimitRuleTable rules={rules} loading={false} hasProvider={false} onToggleMenu={vi.fn()} />
    ));
  ...
  it('renders kebab button with aria-label', () => {
    ...
    render(() => (
      <LimitRuleTable rules={rules} loading={false} hasProvider={true} onToggleMenu={vi.fn()} />
    ));
  ...
  it('shows loading skeleton when loading', () => {
    const { container } = render(() => (
      <LimitRuleTable rules={undefined} loading={true} hasProvider={false} onToggleMenu={vi.fn()} />
    ));
  ...
  it('shows empty state when no rules', () => {
    render(() => (
      <LimitRuleTable rules={[]} loading={false} hasProvider={false} onToggleMenu={vi.fn()} />
    ));
  ...
  it.each([
    ...
    [2, '2 токена'],
    [5, '5 токенов'],
  ] as const)('uses the Russian plural form for %s tokens', async (threshold, expected) => {
    await setLocale('ru');
    const { container } = render(() => (
      <LimitRuleTable
        rules={[makeRule({ threshold })]}
        loading={false}
        hasProvider={true}
        onToggleMenu={vi.fn()}
      />
