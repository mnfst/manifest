...
vi.mock('@solidjs/router', () => ({
  ...
  useLocation: () => ({ pathname: mockPathname, state: mockLocationState }),
}));
...
import { clearBreadcrumb, setBreadcrumb } from '../../src/services/breadcrumb-store';
...
describe('Header', () => {
  ...
  it('opens dropdown on click', async () => {
    render(() => <Header />);
    await fireEvent.click(screen.getByLabelText('Account menu'));
...
describe('Header - docs link', () => {
  ...
  it('keeps the legacy Limits route mapped to limits docs', () => {
    mockPathname = '/agents/my-agent/limits';
    const { container } = render(() => <Header />);
    expect(docsHref(container)).toBe('https://manifest.build/docs/set-limits');
  });
...
describe('Header - teams breadcrumb', () => {
  it('shows the account name fallback when the session has no name', () => {
    // "User" is a person the company tracks spend for; the login is the account.
    const { container } = render(() => <Header />);
    expect(container.querySelector('.header__dropdown-name')).toBeNull();
    expect(screen.getByLabelText('Account menu')).toBeDefined();
  });

  it('starts the agent breadcrumb at Agents by default', () => {
    mockAgentName = 'claude-code';
    const { container } = render(() => <Header />);
    const links = Array.from(container.querySelectorAll('a.header__breadcrumb-link')).map((a) => [
      a.getAttribute('href'),
      a.textContent,
    ]);
    expect(links).toEqual([['/agents', 'Agents']]);
    expect(container.querySelector('.header__breadcrumb-current')?.textContent).toContain(
      'claude-code',
    );
  });

  it('reads the trail the person arrived by from the router state', () => {
    mockAgentName = 'claude-code';
    mockAgentDisplayName = 'Claude Code';
    mockLocationState = {
      via: [
  ...
  it('ignores a malformed via state', () => {
    mockAgentName = 'claude-code';
    mockLocationState = { via: 'nope' };
    const { container } = render(() => <Header />);
    expect(container.querySelectorAll('a.header__breadcrumb-link').length).toBe(1);
  });

  it('renders the store-driven trail for user and project pages, with an icon when given', () => {
    setBreadcrumb([{ label: 'Projects', href: '/projects' }], {
      label: 'HSBC',
      icon: '/icons/x.svg',
    });
    const { container } = render(() => <Header />);
    const link = container.querySelector('a.header__breadcrumb-link');
    expect(link?.getAttribute('href')).toBe('/projects');
    expect(link?.textContent).toBe('Projects');
    const current = container.querySelector('.header__breadcrumb-current');
    expect(current?.textContent).toContain('HSBC');
    expect(current?.querySelector('img.header__breadcrumb-icon')?.getAttribute('src')).toBe(
      '/icons/x.svg',
    );
    // No gear menu outside an agent page.
    expect(container.querySelector('.header__gear')).toBeNull();
  });

  it('renders the store-driven current page without an icon', () => {
    setBreadcrumb([{ label: 'Users', href: '/users' }], { label: 'Maya Okonkwo' });
    const { container } = render(() => <Header />);
    const current = container.querySelector('.header__breadcrumb-current');
    expect(current?.textContent).toContain('Maya Okonkwo');
    expect(current?.querySelector('img')).toBeNull();
  });

  it('lets the store-driven breadcrumb win over a stale connection breadcrumb', () => {
    setConnectionBreadcrumb('OpenAI Default', '/providers/usage-based', 'Usage-based', 'openai');
    setBreadcrumb([{ label: 'Users', href: '/users' }], { label: 'Maya Okonkwo' });
    const { container } = render(() => <Header />);
    expect(container.querySelector('a[href="/providers/usage-based"]')).toBeNull();
  ...
  it('hides the store-driven breadcrumb when an agent is active', () => {
    mockAgentName = 'claude-code';
    setBreadcrumb([{ label: 'Users', href: '/users' }], { label: 'Maya Okonkwo' });
    const { container } = render(() => <Header />);
    expect(container.textContent).not.toContain('Maya Okonkwo');
  });
});
