...
describe('SetupStepAddProvider', () => {
  ...
  describe('platform-filtered mode', () => {
    ...
    it('shows correct heading for pi', () => {
      render(() => <SetupStepAddProvider {...defaultProps} platform="pi" />);
      expect(screen.getByText('Connect Pi to Manifest')).toBeDefined();
    });

    it('shows WarpSetup directly when platform is warp', () => {
      const { container } = render(() => (
        <SetupStepAddProvider {...defaultProps} platform="warp" />
      ));
      expect(container.textContent).toContain('custom inference endpoint');
    ...
    it('shows correct heading for warp', () => {
      render(() => <SetupStepAddProvider {...defaultProps} platform="warp" />);
      expect(screen.getByText('Connect Warp to Manifest')).toBeDefined();
    });
