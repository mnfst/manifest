...
describe("SetupStepAddProvider", () => {
  ...
  it("shows OpenClaw, Hermes, Nanobot, Craft, Claude Code, OpenCode, and Codex CLI tabs inside Agents", () => {
    const { container } = render(() => <SetupStepAddProvider {...defaultProps} />);
    ...
    expect(agentTabs[6].textContent).toContain("Codex CLI");
  });
  ...
  it("shows Codex CLI setup when Codex CLI tab clicked", () => {
    const { container } = render(() => <SetupStepAddProvider {...defaultProps} />);
    const agentTabs = container.querySelectorAll(".panel__tab");
    fireEvent.click(agentTabs[6]); // Codex CLI
  ...
  describe("platform-filtered mode", () => {
    ...
    it("shows correct heading for codex", () => {
      ...
      expect(screen.getByText("Connect Codex CLI to Manifest")).toBeDefined();
    });
