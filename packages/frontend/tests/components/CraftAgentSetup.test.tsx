...
describe('CraftAgentSetup', () => {
  ...
  it('renders the Russian instruction with locale-specific grammar and rich code nodes', async () => {
    await setLocale('ru');
    try {
      const { container } = render(() => <CraftAgentSetup {...baseProps} />);
      const instruction = container.querySelector('.setup-method__hint')!;
      expect(instruction.textContent).toBe(
        'Добавьте в Craft подключение по API-ключу и выберите шаблон провайдера Manifest. Укажите модель auto и вставьте API-ключ:',
      );
      const codeNodes = instruction.querySelectorAll('code.setup-model-hint__code');
      expect([...codeNodes].map((node) => node.textContent)).toEqual(['Manifest', 'auto']);
    } finally {
      await setLocale('en');
    }
  });
  ...
  it('masks the API key when only a prefix is available', () => {
    const { container } = render(() => <CraftAgentSetup {...baseProps} keyPrefix="mnfst_abc" />);
