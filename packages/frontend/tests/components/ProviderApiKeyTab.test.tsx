...
describe('ProviderApiKeyTab', () => {
  it('sorts standard and custom providers alphabetically in the merged list', async () => {
    ...
    const { container } = render(() => (
      <ProviderApiKeyTab
        apiKeyProviders={[
          provider({ id: 'zee', name: 'Zeta' }),
          provider({ id: 'ant', name: 'Anthropic' }),
        ]}
        customProviders={[
          { id: 'c1', name: 'Custom-B', base_url: 'https://b', models: [] } as never,
        ]}
        isConnected={() => false}
        isNoKeyConnected={() => false}
        onOpenDetail={vi.fn()}
  ...
  it('hides the empty LM Studio tile when a custom provider already claims that canonical name', async () => {
    ...
    // local-LLM.
    const lmsRows = Array.from(container.querySelectorAll('.provider-toggle__name')).filter((n) =>
      n.textContent?.includes('LM Studio'),
    );
