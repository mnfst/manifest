    ...
    return (
      <div data-testid={`combo-${id}`}>
        <input
          ...
          onInput={(e) =>
            (props.onInput as (v: string) => void)((e.currentTarget as HTMLInputElement).value)
          }
        />
        {props.errorMessage ? (
...
describe('HeaderTierModal', () => {
  beforeEach(() => {
    ...
    mockCreateHeaderTier.mockResolvedValue({
      ...existingTier,
      id: 'ht-new',
      name: 'Created',
      response_mode: 'buffered',
  ...
  describe('create mode', () => {
    ...
    it('invokes onClose when the close button is clicked', () => {
      const onClose = vi.fn();
      const { container } = render(() => (
        <HeaderTierModal agentName="demo" existingTiers={[]} onClose={onClose} onSaved={vi.fn()} />
      ));
    ...
    it('submits the create payload when the form is valid', async () => {
      ...
      const onClose = vi.fn();
      const { container } = render(() => (
        <HeaderTierModal agentName="demo" existingTiers={[]} onClose={onClose} onSaved={onSaved} />
      ));
    ...
    it('rejects a header key with invalid characters', async () => {
      ...
      await waitFor(() => {
        expect(
          container.querySelector('[data-testid="combo-error-header-tier-key"]')?.textContent,
    ...
    it('rejects header values containing quotes', async () => {
      ...
      await waitFor(() => {
        expect(
          container.querySelector('[data-testid="combo-error-header-tier-value"]')?.textContent,
        ).toMatch(/quotes or backslashes/);
      });
    });
    ...
    it("rejects a value that duplicates an existing tier's key+value", async () => {
      ...
      await waitFor(() => {
        expect(
          container.querySelector('[data-testid="combo-error-header-tier-value"]')?.textContent,
        ).toMatch(/Another tier already matches/);
      });
    });
  ...
  describe('stream mode toggle', () => {
    ...
    it('persists stream mode change via setHeaderTierResponseMode on save', async () => {
      ...
      await waitFor(() => {
        ...
        expect(mockSetHeaderTierResponseMode).toHaveBeenCalledWith(
          'demo',
          existingTier.id,
          'stream',
        );
  ...
  it('emits the count-only sublabel for keys with no top_values', async () => {
    ...
    await waitFor(() => {
      ...
      const suggestions = lastKeyCall?.suggestions as Array<{ sublabel?: string }> | undefined;
  ...
  it('locale-formats large seen-header counts', async () => {
    ...
    try {
      ...
      await waitFor(() => {
        const lastKeyCall = comboCalls.find((c) => c.id === 'header-tier-key');
        const suggestions = lastKeyCall?.suggestions as Array<{ sublabel?: string }> | undefined;
        expect(suggestions?.[0]?.sublabel).toMatch(/^Количество совпадений: 12[\u00a0\u202f]345$/);
      });
    } finally {
      await setLocale('en');
    }
  ...
  describe('stream mode and incompatible models', () => {
    ...
    it('shows singular blocker text when only one model is incompatible', () => {
      ...
      const modelsWithCaps = [{ model_name: 'gpt-4o', capabilities: ['text'] as const }];
      const { container } = render(() => (
        <HeaderTierModal
          agentName="demo"
    ...
    it('handles models with capabilities set on available models (caps.set path)', () => {
      const modelsWithCaps = [{ model_name: 'gpt-4o', capabilities: ['text', 'stream'] as const }];
  ...
  describe('keyboard shortcuts', () => {
    ...
    it('does not submit when Enter event was already handled (defaultPrevented)', () => {
      const onSaved = vi.fn();
      const { container } = render(() => (
        <HeaderTierModal agentName="demo" existingTiers={[]} onClose={vi.fn()} onSaved={onSaved} />
      ));
      const nameInput = container.querySelector('#header-tier-name') as HTMLInputElement;
