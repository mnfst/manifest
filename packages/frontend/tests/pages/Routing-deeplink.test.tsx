...
vi.mock('@solidjs/router', () => ({
  ...
  useSearchParams: () => [{ provider: 'anthropic' }, mockSetSearchParams],
  useNavigate: () => vi.fn(),
    ...
    <>
      {props.showProviderModal() && (
        <div
          ...
          data-deeplink={JSON.stringify(props.providerDeepLink ?? null)}
          data-prefill={JSON.stringify(props.customProviderPrefill ?? null)}
        >
...
describe('Routing — providerDeepLink', () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it('opens provider modal automatically when search params have provider=anthropic', async () => {
    render(() => <Routing />);
    await waitFor(() => {
  ...
  it('passes providerDeepLink to the provider modal', async () => {
    render(() => <Routing />);
    await waitFor(() => {
      const modal = screen.getByTestId('provider-modal');
      const deepLink = JSON.parse(modal.getAttribute('data-deeplink') ?? 'null');
      expect(deepLink).toEqual({ providerId: 'anthropic' });
    });
  });

  it('does not pass customProviderPrefill for non-custom provider', async () => {
    render(() => <Routing />);
    await waitFor(() => {
      const modal = screen.getByTestId('provider-modal');
