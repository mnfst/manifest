...
vi.mock('../../src/components/ProviderSelectModal.js', () => ({
  default: (props: any) => (
    <div
      ...
      data-custom-providers={JSON.stringify(props.customProviders ?? [])}
    >
      <button onClick={props.onClose}>Done</button>
      <button onClick={props.onUpdate} data-testid="trigger-update">
        Update
      </button>
    </div>
  ),
...
vi.mock('../../src/services/api.js', () => ({
  ...
  getPricingHealth: vi
    .fn()
    .mockResolvedValue({ model_count: 100, last_fetched_at: '2026-04-13T00:00:00.000Z' }),
  refreshPricing: vi
    .fn()
...
describe('Routing — enabled state (providers active)', () => {
  ...
  it('selects a model and calls overrideTier', async () => {
    ...
    await waitFor(() => {
      expect(overrideTier).toHaveBeenCalledWith(
        'test-agent',
        'simple',
        'claude-opus-4-6',
        'anthropic',
  ...
  it('shows Resetting... and disables button during resetAllTiers', async () => {
    ...
    vi.mocked(resetAllTiers).mockReturnValue(
      new Promise<void>((r) => {
        resolveResetAll = r;
      }) as any,
    );
...
describe('Routing — pricing cache health banner', () => {
  ...
  it('does not show the banner when the pricing cache has models', async () => {
    vi.mocked(getPricingHealth).mockResolvedValue({
      model_count: 100,
  ...
  it('calls refreshPricing when the retry button is clicked', async () => {
    vi.mocked(getPricingHealth).mockResolvedValue({ model_count: 0, last_fetched_at: null });
    vi.mocked(refreshPricing).mockResolvedValue({
      ok: true,
      model_count: 200,
...
describe('Routing — helper functions', () => {
  ...
  it('closes confirm disable modal on overlay click', async () => {
    ...
    const confirmOverlay = Array.from(overlays).find((o) =>
      o.textContent?.includes('Disable routing?'),
    )!;
    ...
    vi.mocked(getTierAssignments).mockResolvedValueOnce([
      {
        ...
        user_id: 'u1',
        tier: 'simple',
        override_model: 'totally-unknown-model-xyz',
        auto_assigned_model: null,
        fallback_models: null,
        updated_at: '2025-01-01',
      },
...
describe('Routing — custom providers', () => {
  ...
  it('renders custom provider icon in routing card when model is custom', async () => {
    ...
    vi.mocked(getTierAssignments).mockResolvedValue([
      {
        ...
        tier: 'simple',
        override_model: null,
        auto_assigned_model: 'custom:cp-uuid/my-llama',
        fallback_models: null,
        updated_at: '2025-01-01',
      },
      {
    ...
    vi.mocked(getAvailableModels).mockResolvedValue([
      {
        model_name: 'custom:cp-uuid/my-llama',
        provider: 'custom:cp-uuid',
...
describe('ModelPickerModal — custom providers and filtering', () => {
  ...
  it('shows custom provider group with letter icon', () => {
    ...
    const groupHeader = container.querySelector(
      '.routing-modal__group-header .provider-card__logo-letter',
    );
...
describe('Routing — fallback management', () => {
  ...
  it('does not duplicate existing fallback model', async () => {
    ...
    const standaloneAddBtn = container.querySelector(
      '.fallback-list__add:not(.fallback-list__empty .fallback-list__add)',
  ...
  it('shows Adding... on the add button while setFallbacks is in progress', async () => {
    ...
    vi.mocked(setFallbacks).mockReturnValueOnce(
      new Promise<void>((r) => {
        resolveSetFallbacks = r;
      }) as any,
    );
    render(() => <Routing />);
    const addButtons = await screen.findAllByText('Add fallback');
  ...
  it('exercises FallbackList props (agentName, tier, customProviders) via remove', async () => {
    const {
      getTierAssignments,
      getAvailableModels,
      clearFallbacks: mockClearFallbacks,
    } = await import('../../src/services/api.js');
    vi.mocked(getTierAssignments).mockResolvedValueOnce([
      {
        id: '1',
        ...
        override_model: null,
        auto_assigned_model: 'gpt-4o-mini',
        fallback_models: ['custom:cp-1/my-llama'],
        updated_at: '2025-01-01',
      },
      {
        id: '2',
    ...
    vi.mocked(getAvailableModels).mockResolvedValueOnce([
      ...
      {
        model_name: 'custom:cp-1/my-llama',
    ...
    await waitFor(() => {
      ...
      const letterIcon = container.querySelector(
        '.fallback-list__card .provider-card__logo-letter',
      );
    ...
    vi.mocked(getTierAssignments).mockResolvedValue([
      {
        ...
        tier: 'simple',
        override_model: null,
        auto_assigned_model: 'qwen-2.5-72b-instruct',
        fallback_models: null,
        updated_at: '2025-01-01',
      },
      {
    ...
    vi.mocked(getTierAssignments).mockResolvedValue([
      {
        ...
        tier: 'simple',
        override_model: null,
        auto_assigned_model: 'my-unknown-model',
        fallback_models: null,
        updated_at: '2025-01-01',
      },
      {
...
describe('Routing — specificity routing', () => {
  ...
  it('handleAddFallback routes to specificity API when tier is a specificity category', async () => {
    ...
    await waitFor(() => {
      expect(setSpecificityFallbacks).toHaveBeenCalledWith(
        'test-agent',
        'coding',
        expect.any(Array),
      );
  ...
  it('onFallbackUpdate for specificity calls setSpecificityFallbacks', async () => {
    ...
    const removeBtn = container.querySelector(
      '.specificity-section .fallback-list__remove',
    ) as HTMLButtonElement;
    if (removeBtn) {
      fireEvent.click(removeBtn);
      await waitFor(() => {
