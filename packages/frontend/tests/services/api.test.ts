...
describe("connectProvider", () => {
  it("POSTs to /routing/providers with JSON body", async () => {
    const payload = { id: "1", provider: "openai", is_active: true };
    mockMutateOk(payload);

    const result = await connectProvider({ provider: "openai", apiKey: "sk-test" });
    expect(result).toEqual(payload);
    expect(mockFetch).toHaveBeenCalledWith(
  ...
  it("throws and shows toast on error", async () => {
    const { toast } = await import("../../src/services/toast-store.js");
    mockMutateError(400, "Invalid API key");

    await expect(connectProvider({ provider: "openai", apiKey: "" })).rejects.toThrow("Invalid API key");
