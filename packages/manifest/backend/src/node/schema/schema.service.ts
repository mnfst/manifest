...
export class SchemaService {
  ...
  async testApiRequest(
    ...
    const { resolved: resolvedUrl, unresolvedVars: urlVars } =
      resolveTemplateVariables(rawUrl, request.mockValues);
    ...
    for (const header of rawHeaders) {
      if (header.key && header.value) {
        const { resolved, unresolvedVars } =
          resolveTemplateVariables(header.value, request.mockValues);
    ...
    const urlValidation = parseAndValidateUrl(resolvedUrl);
