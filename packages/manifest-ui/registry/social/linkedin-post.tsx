...
export interface LinkedInPostProps {
  ...
  actions?: {
    ...
    repostUrl?: string
    /** Callback when "more" button is clicked to expand content. */
...
const reactionIcons: Record<ReactionType, JSX.Element> = {
  ...
  support: (
    <svg viewBox="0 0 48 48" className="h-5 w-5">
      ...
      <path d="M9.2 27.44c-.66-1.93.42-2.94 1.64-2.94s2.5 1.22 4.54 2.59A48.66 48.66 0 0022 30.5" fill="none" stroke="#493d57" strokeWidth="2"/>
  ...
  love: (
    <svg viewBox="0 0 48 48" className="h-5 w-5">
      ...
      <path d="M23 15.71a9.93 9.93 0 01.84 1.2 1 1 0 001.72-1 13 13 0 00-1.15-1.6 1 1 0 00-1.76.39 1 1 0 00.35 1.01z" fill="#77280c" fillRule="evenodd"/>
    </svg>
  ),
  insightful: (
    <svg viewBox="0 0 48 48" className="h-5 w-5">
  ...
  funny: (
    <svg viewBox="0 0 48 48" className="h-5 w-5">
      ...
      <path d="M33.44,21.34c-1.46-2.65-4.92-3.39-7.35-1.59l-.28,.21,.41,.89h0c2.23-.52,4.59-.11,6.51,1.15l.71-.66Z" fill="#104e58"/>
...
export function LinkedInPost({ data, actions, appearance }: LinkedInPostProps) {
  ...
  const { postUrl, repostUrl, onExpandContent } = actions ?? {}
