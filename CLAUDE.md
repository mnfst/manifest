...
- `MANIFEST_ENCRYPTION_KEY_PREVIOUS` — Optional, rotation only. A secret that stored values may still be encrypted under. Decryption tries `MANIFEST_ENCRYPTION_KEY`, then this, then `BETTER_AUTH_SECRET`; on boot, one replica rewrites every row found under an older secret onto the current one (`database/secret-reencryption.service.ts`, advisory-locked, never fails boot). Rotation: set the new `MANIFEST_ENCRYPTION_KEY` and `MANIFEST_ENCRYPTION_KEY_PREVIOUS=<old>`, deploy, wait for the boot log line `Nothing left under an older secret`, then remove `PREVIOUS`. Moving off the `BETTER_AUTH_SECRET` fallback needs no `PREVIOUS`: the session secret is always tried last.
